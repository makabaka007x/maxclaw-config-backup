...
**Step 3 → Step 4**: Main Agent uses `minimax-docx` skill to generate professionally formatted DOCX based on {topic}_report_verified.md, then converts DOCX to PDF.
...
**REQUIRED:**
- ✅ Complete researcher → report_writer → fact_checker → minimax-docx skill + PDF conversion
...
1. Uses the `minimax-docx` skill to generate professional DOCX from `{topic}_report_verified.md`
